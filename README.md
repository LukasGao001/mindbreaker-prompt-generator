...
- **思维模型匹配**：内置 24 个思维模型（逆向思维、第一性原理、安全边际……）+ 12 个 Prompt 框架（角色扮演、思维链、对比多方案……）。描述任务后，先识别**任务类型**（流程/决策/分析/规划/创作/沟通），再据此加权推荐贴合任务本质的模型，可勾选增减。
- **生成 Skill（SKILL.md）**：对"标准 / 流程 / 执行"类任务，一键把上下文 + 思维原则 + SOP 步骤 + 检查清单沉淀为可复用的 `SKILL.md`（YAML 头部 + Markdown 正文），支持复制与下载。
